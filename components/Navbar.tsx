...
import { Mountain, Moon, Sun, Lock, KeyRound, LogOut } from 'lucide-react';
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[115] pt-4 md:pt-6 px-4 select-none pointer-events-none">
      <div className="max-w-7xl mx-auto flex justify-between items-center relative">
        ...
        <button
          onClick={() => { vibrate(10); smoothScrollTo('home', 0); }}
          ...
          <nav className="glass-panel bg-black/40 backdrop-blur-xl rounded-full px-2 py-1.5 flex items-center gap-1 shadow-2xl ring-1 ring-white/10 dark:ring-white/10">
            {navLinks.map((key) => (
              <button
                ...
                onClick={(e) => {
                  e.preventDefault();
                  smoothScrollTo(key === 'reviews' ? 'reviews' : key, 80);
                }}
        ...
        <div className="hidden md:flex items-center gap-2 pointer-events-auto">
          <div className="glass-panel bg-black/40 backdrop-blur-xl p-1.5 rounded-full flex gap-1 ring-1 ring-white/10 dark:ring-white/10">
            ...
            {isAdmin ? (
              <button
                ...
                className="p-2.5 rounded-full hover:bg-red-500/20 text-emerald-400 hover:text-red-400 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center"
                aria-label="Выйти"
              ...
              <button
                onClick={() => { vibrate(10); setShowLoginModal(true); }}
                className="p-2.5 rounded-full hover:bg-white/10 text-white/40 hover:text-white transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center"
          ...
          <button
            onClick={(e) => {
              e.preventDefault();
              smoothScrollTo('contacts', 80);
            }}
            className="bg-gray-900 dark:bg-white text-white dark:text-gray-900 px-6 py-3.5 rounded-full text-xs font-bold uppercase tracking-wider hover-hover:hover:scale-105 active:scale-95 transition-all shadow-xl min-h-[44px] flex items-center"
          >
            {t.nav.contacts}
          </button>
        </div>
      ...
      <div className={`fixed inset-0 z-[110] h-[100dvh] bg-white/98 dark:bg-gray-950/98 backdrop-blur-2xl transition-all duration-500 flex flex-col justify-center items-center px-8 md:hidden pointer-events-auto ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
        ...
        <div className={`flex gap-4 w-full max-w-sm transition-all duration-700 delay-200 ${isOpen ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          ...
          {isAdmin ? (
            <button
              onClick={() => { vibrate(10); signOut(); setIsOpen(false); }}
              className="h-16 w-16 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400 active:scale-95 transition-transform"
            ...
            <button
              onClick={() => { vibrate(10); setIsOpen(false); setShowLoginModal(true); }}
              className="h-16 w-16 rounded-2xl bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 flex items-center justify-center text-gray-400 dark:text-white/40 active:scale-95 transition-transform"
              aria-label="Вход для админа"
            >
