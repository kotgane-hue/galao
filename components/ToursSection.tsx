...
import { Clock, Users, ArrowUpRight, Pencil, Trash2, Plus } from 'lucide-react';
import { Tour, TourCategory } from '../types';
import { useLanguage } from '../contexts/LanguageContext';
import { useAuth } from '../contexts/AuthContext';
import { vibrate } from '../utils/vibrate';
import TourEditModal from './admin/TourEditModal';
...
const TourCard = React.memo(({ tour, onClick, isSelected, priority, isAdmin, onEdit, onDelete }: { tour: Tour; onClick: () => void; isSelected: boolean; priority: boolean; isAdmin: boolean; onEdit: (tour: Tour) => void; onDelete: (id: string) => void }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-6 left-6 right-6 flex justify-between items-start z-20">
        <span className={`px-3 py-1.5 rounded-full bg-black/40 backdrop-blur-md text-white text-[10px] font-bold uppercase tracking-widest border border-white/10 shadow-sm pointer-events-none
        ...
        <div className="flex items-center gap-2">
          ...
          {isAdmin && (
            <>
              <button
                ...
                className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-emerald-500/80 backdrop-blur-md text-white flex items-center justify-center shadow-lg hover:bg-emerald-600 transition-colors pointer-events-auto"
                title="Редактировать"
              >
                <Pencil className="w-3.5 h-3.5 md:w-4 md:h-4" />
              </button>
              <button
                onClick={(e) => { e.stopPropagation(); if (confirm(`Удалить тур "${tour.title}"?`)) onDelete(tour.id); }}
                className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-red-500/80 backdrop-blur-md text-white flex items-center justify-center shadow-lg hover:bg-red-600 transition-colors pointer-events-auto"
                title="Удалить"
              >
          ...
          <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-white text-black flex items-center justify-center transform md:rotate-45 md:group-hover:rotate-0 transition-transform duration-500 shadow-lg pointer-events-none">
...
const ToursSection: React.FC<ToursSectionProps> = ({ onOpenTour, selectedTourId }) => {
  const { tours, t, deleteTour } = useLanguage();
  const { isAdmin } = useAuth();
  const [activeFilter, setActiveFilter] = useState<'all' | TourCategory>('all');
  const [visibleCount, setVisibleCount] = useState(6);
  const [editingTour, setEditingTour] = useState<Tour | null | undefined>(undefined); // undefined = closed, null = new, Tour = editing
  ...
  return (
    <section className="py-24 md:py-32 bg-white dark:bg-black transition-colors duration-500 relative z-10">
      <div className="max-w-7xl mx-auto px-6">
        ...
        {/* Admin: Add New Tour Button */}
        {isAdmin && (
          <button
            onClick={() => setEditingTour(null)}
            className="mb-6 w-full py-6 rounded-[2rem] border-2 border-dashed border-emerald-500/40 text-emerald-400 hover:border-emerald-500 hover:bg-emerald-500/5 transition-all flex items-center justify-center gap-3 font-bold text-sm uppercase tracking-widest"
          >
      ...
      {editingTour !== undefined && (
        <TourEditModal tour={editingTour} onClose={() => setEditingTour(undefined)} />
      )}
    </section>
  );
};
