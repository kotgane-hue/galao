...
import { Quote, Instagram, MessageSquare, X, Send, Utensils, Pencil, Trash2, Plus } from 'lucide-react';
...
const TeamSection: React.FC = () => {
  const { team, t, deleteTeamMember } = useLanguage();
  ...
  return (
    <section
      ...
      className={`py-4 md:py-32 min-h-[100dvh] flex flex-col justify-center overflow-hidden relative transition-all duration-700 ease-in-out select-none ${isAvocadoActive
    ...
    >
              ...
              <div className="absolute inset-0 rounded-[2.5rem] overflow-hidden shadow-2xl transition-transform duration-700 hover-hover:group-hover:scale-[1.02] hover-hover:group-hover:-translate-y-2 z-10">
                ...
                {team.map((member, idx) => (
                  <img
                    ...
                    src={member.image}
                    alt={member.name}
                    className={`absolute inset-0 w-full h-full object-cover object-top transition-all duration-700 ease-out ${idx === activeIndex ? 'opacity-100 scale-100' : 'opacity-0 scale-110'
                      }`}
                  />
                ))}
            ...
            <div className="w-full md:w-7/12 flex flex-col justify-center order-2 md:order-1 relative z-20">
              ...
              <div key={activeIndex} className="animate-fade-in-up">
                ...
                <div className="flex flex-wrap items-center gap-2 md:gap-4">
                  ...
                  {/* Admin: Edit/Delete/Add Team Member */}
                  {isAdmin && (
                    <>
                      <button
                        onClick={() => setEditingMember(activeMember)}
                        className="px-4 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20 transition-all font-bold text-xs uppercase tracking-wide flex items-center gap-2 min-h-[44px]"
                        title="Редактировать"
                      >
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => { if (confirm(`Удалить ${activeMember.name}?`)) { deleteTeamMember(activeMember.id); setActiveIndex(0); } }}
                        className="px-4 py-3 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 hover:bg-red-500/20 transition-all font-bold text-xs uppercase tracking-wide flex items-center gap-2 min-h-[44px]"
                        title="Удалить"
                      >
                      ...
                      <button
                        ...
                        className="px-4 py-3 rounded-xl bg-blue-500/10 border border-blue-500/30 text-blue-400 hover:bg-blue-500/20 transition-all font-bold text-xs uppercase tracking-wide flex items-center gap-2 min-h-[44px]"
                        title="Добавить"
                      >
              ...
              <div className="hidden md:flex items-center gap-3 mt-16 pt-8 border-t border-gray-100 dark:border-gray-900">
                {team.map((member, idx) => (
                  ...
                  >
                    <div className={`w-full aspect-square rounded-2xl overflow-hidden border-2 transition-all ${idx === activeIndex
      ...
      {editingMember !== undefined && (
        <TeamEditModal member={editingMember} onClose={() => setEditingMember(undefined)} />
      )}
    </section>
  );
};
