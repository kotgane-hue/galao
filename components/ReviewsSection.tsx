...
const ReviewsSection: React.FC<ReviewsSectionProps> = ({ onOpenReviewModal }) => {
  const { reviews, t, deleteReview } = useLanguage();
            ...
            >
              <article className="bg-white dark:bg-gray-900 p-8 rounded-[2.4rem] h-full flex flex-col relative z-10">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  {isAdmin && (
                    <button
                      onClick={() => { if (confirm(`Удалить отзыв от ${review.author}?`)) deleteReview(idx); }}
                      className="p-2 rounded-full bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-colors shrink-0"
                      title="Удалить отзыв"
                    >
