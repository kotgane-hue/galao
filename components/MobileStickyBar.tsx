...
import { Users, Check, Send, Mountain, CalendarCheck } from 'lucide-react';
...
const MobileStickyBar: React.FC<MobileStickyBarProps> = ({ onOpenCalendar }) => {
  ...
  const handleNavClick = (tab: string, hash?: string) => {
    vibrate(10);
    setActiveTab(tab);
    if (hash) smoothScrollTo(hash.replace('#', ''), 80);
  };
  ...
  const NavItem = ({ icon: Icon, label, id, onClick, isActiveOverride }: { icon: any, label: string, id: string, onClick: () => void, isActiveOverride?: boolean }) => {
    ...
    return (
      ...
      >
        ...
        <Icon
          className={`w-6 h-6 z-10 transition-all duration-300 ${isActive
        ...
        {/* Micro-text */}
        <span className={`text-[9px] font-bold uppercase tracking-wider z-10 transition-colors duration-300 ${isActive ? 'text-white' : 'text-gray-600'
          }`}>
          {label}
        </span>
      </button>
  ...
  return (
    <div
      className={`fixed bottom-6 left-4 right-4 z-[90] md:hidden transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-32 opacity-0'
        }`}
    >
      ...
      <div className="relative h-[72px] flex items-center justify-between px-2">
        ...
        <div className="relative -top-5 mx-1 z-20">
          <button
            onClick={(e) => { e.preventDefault(); vibrate(20); setActiveTab('tours'); smoothScrollTo('tours', 80); }}
