...
const Hero: React.FC<HeroProps> = ({ onOpenCalendar }) => {
  ...
  useEffect(() => {
    // Only animate once on the very first mount
    if (!hasAnimated.current) {
      // Small delay so CSS transitions can pick up the change
      requestAnimationFrame(() => {
        setIsLoaded(true);
        hasAnimated.current = true;
      });
    } else {
      // If component was already mounted before, show content immediately
      setIsLoaded(true);
    }
  ...
  const handleScrollToTours = () => {
    vibrate(10);
    // Use the custom smooth scroll that calculates correct layout offsets 
    // even heavily lazy-loaded pages with height shifting.
    smoothScrollTo('tours', 80);
  };
  ...
  return (
    <header id="home" className="relative h-[100dvh] w-full overflow-hidden bg-gray-900 flex flex-col justify-center items-center select-none">
      ...
      <div className="relative z-20 w-full max-w-7xl mx-auto px-6 flex flex-col items-center text-center mt-[-10vh] md:mt-0">
        ...
        <h1 className="font-heading font-black text-white leading-[0.9] tracking-tighter mb-12 md:mb-8 w-full min-h-[10vw] flex items-center justify-center text-balance">
          <div
